...
server.tool("list-superheroes", "Get a list of all superheroes with their IDs and names", {}, async () => {
    const superheroes = await loadSuperheroes();
...
server.tool("compare-superheroes", "Compare two superheroes across all six powerstat dimensions", {
    ...
    let hero1Total = 0;
    let hero2Total = 0;
    const lines = [`Comparing ${hero1.name} vs ${hero2.name}`];
    for (const stat of stats) {
        ...
        lines.push(`${stat}: ${hero1.name} (${val1}) vs ${hero2.name} (${val2}) - Winner: ${winner}`);
    }
    const overallWinner = hero1Total > hero2Total ? hero1.name : hero1Total < hero2Total ? hero2.name : "Tie";
    lines.push(`Overall winner: ${overallWinner} (${Math.max(hero1Total, hero2Total)} total stats)`);
    return {
        content: [
            {
                type: "text",
